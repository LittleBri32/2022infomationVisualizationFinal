...
function updateExtraData(){
    ...
    const data = getJsonData("../data/Earthquake.json");
    ...
    data.then(res=>{
        const filteredData = res.filter(e=>e.Year===`${beginYear}`&&e.Month==`${beginMonth}`);
...
function updateScatterPlot(filteredData){
    ...
    d3.csv("../data/Earthquake.csv").then(function(data) {
        svg.append('g')
        .selectAll("dot")
        .data(data)
        .join("circle")
